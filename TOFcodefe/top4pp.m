...
TOF_gdop_1 = zeros(1, num_steps);
TOF_gdop_2 = zeros(1, num_steps);
TOF_gdop_3 = zeros(1, num_steps);
...
for t = 1:num_steps
    true_target = true_targets_1(:, t);
    measurement_noise = 0.1; % 噪声水平 (ns)
    noisy_tof_times = DS_TWR(measurement_noise, anchors, true_target);
    [TOF_estimates_1(:, t), TOF_gdop_1(t)] = TOF_localization(anchors, noisy_tof_times);
end
...
for t = 1:num_steps
    true_target = true_targets_2(:, t);
    measurement_noise = 0.1; % 噪声水平 (ns)
    noisy_tof_times = DS_TWR(measurement_noise, anchors, true_target);
    [TOF_estimates_2(:, t), TOF_gdop_2(t)] = TOF_localization(anchors, noisy_tof_times);
end
...
for t = 1:num_steps
    true_target = true_targets_3(:, t);
    measurement_noise = 0.1; % 噪声水平 (ns)
    noisy_tof_times = DS_TWR(measurement_noise, anchors, true_target);
    [TOF_estimates_3(:, t), TOF_gdop_3(t)] = TOF_localization(anchors, noisy_tof_times);
end
...
fprintf('第一条轨迹 TOF 定位的平均误差: %.2f mm\n', TOF_avg_error_1);
fprintf('第一条轨迹 UKF 滤波的平均误差: %.2f mm\n', UKF_avg_error_1);
fprintf('第二条轨迹（正方形） TOF 定位的平均误差: %.2f mm\n', TOF_avg_error_2);
fprintf('第二条轨迹（正方形） UKF 滤波的平均误差: %.2f mm\n', UKF_avg_error_2);
fprintf('第三条轨迹（椭圆螺旋线） TOF 定位的平均误差: %.2f mm\n', TOF_avg_error_3);
fprintf('第三条轨迹（椭圆螺旋线） UKF 滤波的平均误差: %.2f mm\n', UKF_avg_error_3);

% 新增GDOP与误差关系图
figure;
subplot(3,1,1);
scatter(TOF_gdop_1, TOF_errors_1, 10, 'filled');
xlabel('GDOP'); ylabel('TOF误差 (mm)'); title('第一条轨迹 GDOP vs 误差');
grid on;

subplot(3,1,2);
scatter(TOF_gdop_2, TOF_errors_2, 10, 'filled');
xlabel('GDOP'); ylabel('TOF误差 (mm)'); title('第二条轨迹 GDOP vs 误差');
grid on;

subplot(3,1,3);
scatter(TOF_gdop_3, TOF_errors_3, 10, 'filled');
xlabel('GDOP'); ylabel('TOF误差 (mm)'); title('第三条轨迹 GDOP vs 误差');
grid on;

% 添加GDOP时间序列图
figure;
plot(1:num_steps, TOF_gdop_1, 'b', 'DisplayName', '轨迹1 GDOP');
hold on;
plot(1:num_steps, TOF_gdop_2, 'r', 'DisplayName', '轨迹2 GDOP');
plot(1:num_steps, TOF_gdop_3, 'g', 'DisplayName', '轨迹3 GDOP');
xlabel('步数'); ylabel('GDOP'); title('GDOP时间变化');
legend; grid on;